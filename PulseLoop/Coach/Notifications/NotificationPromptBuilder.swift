...
enum NotificationPromptBuilder {
    static func systemPrompt(slot: CoachNotificationSlot) -> String {
        ...
        - When a `nutrition` block is present, you may briefly relate intake to the eating goal (e.g. calories left) — matter-of-fact, never shaming, and no false precision on days with estimated meals. `goals.calorie_intake_daily` is the EATING goal; `today.calories` is energy burned.
